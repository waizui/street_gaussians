...
class StreetGaussianModel(nn.Module):
    def __init__(self, metadata):
        ...
        # symmetry
        self.flip_prob = cfg.model.gaussian.get("flip_prob", 0.0)
        self.flip_axis = 1
        ...
        if self.include_obj:
            for track_id, obj_meta in self.obj_info.items():
                ...
                setattr(
                    self,
                    model_name,
                    GaussianModelActor(model_name=model_name, obj_meta=obj_meta),
                )
        ...
        if self.include_obj:
            for i, obj_name in enumerate(self.obj_list):
                ...
                if (
                    ...
                    and self.frame <= end_frame
                    and self.get_visibility(obj_name)
                ):
        ...
        if len(self.graph_obj_list) > 0:
            ...
            for i, obj_name in enumerate(self.graph_obj_list):
                ...
                ego_pose_rot = matrix_to_quaternion(
                    ego_pose[:3, :3].unsqueeze(0)
                ).squeeze(0)
                obj_rot = quaternion_raw_multiply(
                    ego_pose_rot.unsqueeze(0), obj_rot.unsqueeze(0)
                ).squeeze(0)
                obj_trans = ego_pose[:3, :3] @ obj_trans + ego_pose[:3, 3]

                obj_rot = obj_rot.expand(obj_model.get_xyz.shape[0], -1)
                obj_trans = obj_trans.unsqueeze(0).expand(
                    obj_model.get_xyz.shape[0], -1
                )
            ...
            if cfg.mode == "train":
                ...
                for obj_name in self.graph_obj_list:
                    ...
                    if obj_model.deformable or self.flip_prob == 0:
                        ...
                        flip_mask = (
                            torch.rand_like(obj_model.get_xyz[:, 0]) < self.flip_prob
                        )
        ...
        if len(self.graph_obj_list) > 0:
            ...
            if cfg.mode == "train":
                ...
                if len(rotations_flip) > 0:
                    rotations_local[self.flip_mask] = quaternion_raw_multiply(
                        self.flip_matrix, rotations_flip
                    )
    ...
    def get_xyz(self):
        ...
        if self.get_visibility("background"):
            ...
            if self.use_pose_correction:
                xyz_bkgd = self.pose_correction.correct_gaussian_xyz(
                    self.viewpoint_camera, xyz_bkgd
                )
        ...
        if len(self.graph_obj_list) > 0:
            ...
            xyzs_obj = (
                torch.einsum("bij, bj -> bi", obj_rots, xyzs_local) + self.obj_trans
            )
    ...
    def add_densification_stats(self, viewspace_point_tensor, visibility_filter):
        ...
        for model_name in self.graph_gaussian_range.keys():
            ...
            model.xyz_gradient_accum[visibility_model, 0:1] += torch.norm(
                viewspace_point_tensor_grad_model[visibility_model, :2],
                dim=-1,
                keepdim=True,
            )
    ...
    def densify_and_prune(
        self, max_grad, min_opacity, prune_big_points, exclude_list=[]
    ):
        scalars = None
        tensors = None
        for model_name in self.model_name_id.keys():
